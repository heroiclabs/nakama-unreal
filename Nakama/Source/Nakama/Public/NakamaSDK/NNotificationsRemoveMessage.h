...
namespace Nakama {
	...
	class NAKAMA_API NNotificationsRemoveMessage::Builder
	{
	private:
		NNotificationsRemoveMessage message;

	public:
		Builder(std::string notificationId);
		Builder(std::vector<std::string> notificationIds);

		Builder Remove(std::string notificationId);

		NNotificationsRemoveMessage Build();
	};
}
