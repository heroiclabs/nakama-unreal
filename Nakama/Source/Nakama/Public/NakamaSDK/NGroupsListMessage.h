...
namespace Nakama {
	...
	class NAKAMA_API NGroupsListMessage::Builder
	{
	private:
		NGroupsListMessage message;

	public:
		Builder() {}

		Builder PageLimit(int64_t pageLimit);
		Builder OrderByAsc(bool orderByAsc);
		Builder Cursor(NCursor cursor);
		Builder FilterByLang(std::string lang);
		Builder FilterByCreatedAt(int64_t createdAt);
		Builder FilterByCount(int64_t count);

		NGroupsListMessage Build();
	};
}
