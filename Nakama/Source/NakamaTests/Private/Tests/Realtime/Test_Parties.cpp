...
void FNakamaPartiesTestBase::SetupClient2AndRequestJoinParty()
{
	Client2 = CreateClient();
	...
	auto Client2AuthSuccessCallback = [&](UNakamaSession* session)
	{
		...
		Socket2->SetConnectCallback([&]()
		{
			...
			auto JoinPartySuccessCallback = [&]()
			{
				auto ListPartyJoinRequestsSuccessCallback = [&](const FNakamaPartyJoinRequest& JoinRequests)
				{
					TestTrue("List Party Join Requests Test Passed", JoinRequests.Presences.Num() == 1);
					StopTest();
				};

				auto ListPartyJoinRequestsErrorCallback = [&](const FNakamaRtError& Error)
				{
					UE_LOG(LogTemp, Error, TEXT("List Party Join Requests. ErrorMessage: %s"), *Error.Message);
