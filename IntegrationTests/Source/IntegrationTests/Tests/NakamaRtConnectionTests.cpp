 ...
 *
 * Tests FNakamaRtConnection connection lifecycle, request/response round-trips,
 ...
 *
 * Run with:
 *   -ExecCmds="Automation RunTests IntegrationTests.NakamaRtConnection"
 */
...
BEGIN_DEFINE_SPEC(FNakamaRtConnectionConnectionSpec, "IntegrationTests.NakamaRtConnectionTests.Connection",
    ...
    TSharedPtr<FNakamaRtConnection> RtConn;

    FString GenerateId() { return FGuid::NewGuid().ToString(EGuidFormats::DigitsWithHyphens); }

END_DEFINE_SPEC(FNakamaRtConnectionConnectionSpec)

void FNakamaRtConnectionConnectionSpec::Define()
{
    BeforeEach([this]()
    {
        ClientConfig = FNakamaClientConfig{ RtServerKey, RtHost, RtPort, false };
        RtConn = MakeShared<FNakamaRtConnection>();
    });
    ...
    Describe("Success", [this]()
    {
        ...
        {
            RtConn->Connect(MakeConnParams(Session)).Next([this, Done](FNakamaWebSocketConnectionResult Result)
            {
    ...
    Describe("InvalidToken", [this]()
    {
        LatentIt("should fail with a malformed token", [this](const FDoneDelegate& Done)
        {
            ...
            RtConn->Connect(Bad).Next([this, Done](FNakamaWebSocketConnectionResult Result)
            {
    ...
    Describe("Reconnect", [this]()
    {
        ...
        {
            RtConn->Connect(MakeConnParams(Session))
                .Next([this](FNakamaWebSocketConnectionResult First) -> TNakamaFuture<FNakamaWebSocketConnectionResult>
                {
                    ...
                    RtConn->Close();
                    return RtConn->Connect(MakeConnParams(Session));
                })
        ...
        LatentIt("should be usable after reconnect (ping round-trip)", [this](const FDoneDelegate& Done)
        {
            RtConn->Connect(MakeConnParams(Session))
                ...
                .Next([this](FNakamaWebSocketConnectionResult Second) -> TNakamaFuture<FNakamaWebSocketResponse>
                {
                    ...
                    return RtConn->Send(TEXT("ping"), MakeShared<FJsonObject>());
                })
...
END_DEFINE_SPEC(FNakamaRtConnectionPingSpec)

void FNakamaRtConnectionPingSpec::Define()
{
    BeforeEach([this]()
    {
        ClientConfig = FNakamaClientConfig{ RtServerKey, RtHost, RtPort, false };
    ...
    Describe("AutoPing", [this]()
    {
        LatentIt("should not accumulate pending requests (memory leak regression)", [this](const FDoneDelegate& Done)
        {
            ...
            RtConn->Connect(P).Next([this, Done](FNakamaWebSocketConnectionResult CR)
            {
                TestFalse("Connect", CR.ErrorCode != ENakamaWebSocketError::None);
                ...
                TWeakPtr<FNakamaRtConnection> WeakConn = RtConn;
                FTSTicker::GetCoreTicker().AddTicker(
                    FTickerDelegate::CreateLambda([this, Done, WeakConn](float) -> bool
                    {
                        if (TSharedPtr<FNakamaRtConnection> Conn = WeakConn.Pin())
                        {
                            TestEqual(TEXT("Auto-ping must not leak entries into Requests"), Conn->GetPendingRequestCount(), 0);
                        }
                        Done.Execute();
                        return false;
                    }),
        ...
        LatentIt("should handle multiple concurrent pings and resolve all of them", [this](const FDoneDelegate& Done)
        {
            RtConn->Connect(MakeConnParams(Session))
                .Next([this, Done](FNakamaWebSocketConnectionResult CR)
                {
                    ...
                    {
                        RtConn->Send(TEXT("ping"), MakeShared<FJsonObject>())
        ...
        LatentIt("should handle pings interleaved with other requests", [this](const FDoneDelegate& Done)
        {
            RtConn->Connect(MakeConnParams(Session))
                .Next([this, Done](FNakamaWebSocketConnectionResult CR)
                {
                    ...
                    RtConn->Send(TEXT("match_create"), MakeShared<FJsonObject>()).Next(Decrement);
                    RtConn->Send(TEXT("ping"),         MakeShared<FJsonObject>()).Next(Decrement);
                    RtConn->Send(TEXT("ping"),         MakeShared<FJsonObject>()).Next(Decrement);
                });
        });
    });
...
END_DEFINE_SPEC(FNakamaRtConnectionChannelSpec)

void FNakamaRtConnectionChannelSpec::Define()
{
    BeforeEach([this]()
    {
        ClientConfig = FNakamaClientConfig{ RtServerKey, RtHost, RtPort, false };
    ...
    Describe("Join", [this]()
    {
        LatentIt("should join a room channel and receive a channel response", [this](const FDoneDelegate& Done)
        {
            RtConn->Connect(MakeConnParams(Session))
                ...
                {
                    TestFalse("Connect", CR.ErrorCode != ENakamaWebSocketError::None);
                    return RtConn->Send(TEXT("channel_join"), MakeChannelJoinJson(GenerateRoomName()));
                })
                .Next([this, Done](FNakamaWebSocketResponse Resp)
                {
        ...
        LatentIt("should send a message to a room channel and receive an ack", [this](const FDoneDelegate& Done)
        {
            RtConn->Connect(MakeConnParams(Session))
                ...
                .Next([this](FNakamaWebSocketResponse JoinResp) -> TNakamaFuture<FNakamaWebSocketResponse>
                {
                    ...
                    return RtConn->Send(TEXT("channel_message_send"), SendJson);
                })
        ...
        LatentIt("should leave a channel successfully", [this](const FDoneDelegate& Done)
        {
            RtConn->Connect(MakeConnParams(Session))
                ...
                .Next([this](FNakamaWebSocketResponse JoinResp) -> TNakamaFuture<FNakamaWebSocketResponse>
                {
                    ...
                    return RtConn->Send(TEXT("channel_leave"), LeaveJson);
                })
...
END_DEFINE_SPEC(FNakamaRtConnectionMatchSpec)

void FNakamaRtConnectionMatchSpec::Define()
{
    BeforeEach([this]()
    {
        ClientConfig = FNakamaClientConfig{ RtServerKey, RtHost, RtPort, false };
    ...
    Describe("Create", [this]()
    {
        LatentIt("should create a relayed match and receive a match_id", [this](const FDoneDelegate& Done)
        {
            RtConn->Connect(MakeConnParams(Session))
                ...
                {
                    TestFalse("Connect", CR.ErrorCode != ENakamaWebSocketError::None);
                    return RtConn->Send(TEXT("match_create"), MakeShared<FJsonObject>());
                })
                .Next([this, Done](FNakamaWebSocketResponse Resp)
                {
                    WSS_FAIL_ON_ERROR(Resp, Done);
        ...
        LatentIt("should leave a match without error", [this](const FDoneDelegate& Done)
        {
            RtConn->Connect(MakeConnParams(Session))
                ...
                .Next([this](FNakamaWebSocketResponse CreateResp) -> TNakamaFuture<FNakamaWebSocketResponse>
                {
                    ...
                    return RtConn->Send(TEXT("match_leave"), LeaveJson);
                })
                .Next([this, Done](FNakamaWebSocketResponse LeaveResp)
                {
                    WSS_FAIL_ON_ERROR(LeaveResp, Done);
...
END_DEFINE_SPEC(FNakamaRtConnectionMatchmakerSpec)

void FNakamaRtConnectionMatchmakerSpec::Define()
{
    BeforeEach([this]()
    {
        ClientConfig = FNakamaClientConfig{ RtServerKey, RtHost, RtPort, false };
    ...
    Describe("Add", [this]()
    {
        LatentIt("should add to matchmaker and receive a ticket", [this](const FDoneDelegate& Done)
        {
            RtConn->Connect(MakeConnParams(Session))
                ...
                {
                    TestFalse("Connect", CR.ErrorCode != ENakamaWebSocketError::None);
                    return RtConn->Send(TEXT("matchmaker_add"), MakeMatchmakerAddJson());
                })
                .Next([this, Done](FNakamaWebSocketResponse Resp)
                {
                    WSS_FAIL_ON_ERROR(Resp, Done);
        ...
        LatentIt("should add to matchmaker and then cancel with the ticket", [this](const FDoneDelegate& Done)
        {
            RtConn->Connect(MakeConnParams(Session))
                ...
                .Next([this](FNakamaWebSocketResponse AddResp) -> TNakamaFuture<FNakamaWebSocketResponse>
                {
                    ...
                    return RtConn->Send(TEXT("matchmaker_remove"), RemoveJson);
                })
...
END_DEFINE_SPEC(FNakamaRtConnectionStatusSpec)

void FNakamaRtConnectionStatusSpec::Define()
{
    BeforeEach([this]()
    {
        ClientConfig = FNakamaClientConfig{ RtServerKey, RtHost, RtPort, false };
    ...
    Describe("Update", [this]()
    {
        LatentIt("should set a user status string", [this](const FDoneDelegate& Done)
        {
            ...
            RtConn->Connect(MakeConnParams(Session))
                ...
                {
                    TestFalse("Connect", CR.ErrorCode != ENakamaWebSocketError::None);
                    return RtConn->Send(TEXT("status_update"), Json);
                })
                .Next([this, Done](FNakamaWebSocketResponse Resp)
                {
                    WSS_FAIL_ON_ERROR(Resp, Done);
        ...
        LatentIt("should clear user status by omitting the status field", [this](const FDoneDelegate& Done)
        {
            ...
            RtConn->Connect(MakeConnParams(Session))
                ...
                {
                    TestFalse("Connect", CR.ErrorCode != ENakamaWebSocketError::None);
                    return RtConn->Send(TEXT("status_update"), MakeShared<FJsonObject>());
                })
                .Next([this, Done](FNakamaWebSocketResponse Resp)
                {
                    WSS_FAIL_ON_ERROR(Resp, Done);
    ...
    Describe("Follow", [this]()
    {
        LatentIt("should follow a user ID without error", [this](const FDoneDelegate& Done)
        {
            ...
            RtConn->Connect(MakeConnParams(Session))
                ...
                {
                    TestFalse("Connect", CR.ErrorCode != ENakamaWebSocketError::None);
                    return RtConn->Send(TEXT("status_follow"), Json);
                })
                .Next([this, Done](FNakamaWebSocketResponse Resp)
                {
                    WSS_FAIL_ON_ERROR(Resp, Done);
        ...
        LatentIt("should follow and then unfollow a user ID", [this](const FDoneDelegate& Done)
        {
            ...
            RtConn->Connect(MakeConnParams(Session))
                ...
                {
                    TestFalse("Connect", CR.ErrorCode != ENakamaWebSocketError::None);
                    return RtConn->Send(TEXT("status_follow"), FollowJson);
                })
                .Next([this, FakeUserId](FNakamaWebSocketResponse FollowResp) -> TNakamaFuture<FNakamaWebSocketResponse>
                {
                    ...
                    return RtConn->Send(TEXT("status_unfollow"), UnfollowJson);
                })
...
END_DEFINE_SPEC(FNakamaRtConnectionDelegatesSpec)

void FNakamaRtConnectionDelegatesSpec::Define()
{
    BeforeEach([this]()
    {
        ClientConfig = FNakamaClientConfig{ RtServerKey, RtHost, RtPort, false };
        ...
        LatentIt("should fire the MessageSent delegate when a message is dispatched", [this](const FDoneDelegate& Done)
        {
            RtConn->Connect(MakeConnParams(Session))
                .Next([this, Done](FNakamaWebSocketConnectionResult CR)
                {
                    ...
                    *Handle = RtConn->MessageSent.AddLambda(
        ...
        LatentIt("should fire ServerResponseReceived when the server replies", [this](const FDoneDelegate& Done)
        {
            RtConn->Connect(MakeConnParams(Session))
                .Next([this, Done](FNakamaWebSocketConnectionResult CR)
                {
                    ...
                    *Handle = RtConn->ServerResponseReceived.AddLambda(
                        [this, Done, Handle, bFired](const FString& Message)
                        {
                            if (*bFired) return;
                            *bFired = true;
                    ...
                    RtConn->Send(TEXT("match_create"), MakeShared<FJsonObject>());
                });
        });
...
END_DEFINE_SPEC(FNakamaRtConnectionDataRaceSpec)

void FNakamaRtConnectionDataRaceSpec::Define()
{
    BeforeEach([this]()
    {
        ClientConfig = FNakamaClientConfig{ RtServerKey, RtHost, RtPort, false };
