...
func MakeViewModelFactory(typePrefix, namespace string) func(any, schema.Api) (any, error) {
	return func(tmRaw any, api schema.Api) (any, error) {
		...
		return ViewModel{
			...
			rtOperations: resolveRtOperations(api, tm, typePrefix),
		}, nil
	}
}
...
func resolveRtOperations(api schema.Api, tm TypeMap, typePrefix string) []RtOperation {
	...
	for _, oneof := range envelope.OneofFields {
		...
		for _, f := range msg.Fields {
			op.Fields = append(op.Fields, resolveField(f.Name, f.Type, f.Repeated, f.Comment.Message(), tm, api, typePrefix))
		}
		for _, f := range msg.MapFields {
			op.MapFields = append(op.MapFields, resolveMapField(f.Name, f.Type, f.Comment.Message(), tm, typePrefix))
		}
